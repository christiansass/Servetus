...
**Server infrastructure (WordInFilm node)**
...
- `10-System/deploy-services-wordinfilm.sh` — SSH deployment script for systemd user services on WordInFilm.
...
- `docker-compose.binaryranch.yml` — CPU-only stack for R420 (no CUDA).
...
- `ansible/deploy-binaryranch.yml` + `ansible/deploy-wordinfilm.yml` — Ansible playbooks for both nodes.
